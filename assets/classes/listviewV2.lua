...
function ListView:init(params)
	...
	for i = 1, #data do
		...
		finalData[i].height = math.floor(data[i]:getHeight())
		finalData[i].xInit = 0
		finalData[i].yInit = prevY + prevH
		prevY = finalData[i].yInit
		prevH = finalData[i].height
		self.listHeight += finalData[i].height
	end

	if(self.cfg.bgColor ~= nil) then
		self:setFillStyle(Shape.SOLID, self.cfg.bgColor, self.cfg.bgAlpha)
	end
	...
	-- create defined shape (captures touch input)
	self.mask = Shape.new()
	self.mask:beginPath()
...
function ListView:scrollList(event)
	if math.abs(velocity) < 0.3 then
		...
		if self.cfg.rowSnap then
			...
			if firstVisibleItem ~= nil
					and self.listItems:getY() < 0
					and self.listHeight - lastItem:getY() - lastItem:getHeight() > 0 then
				...
				self.tween = GTween.new(
					self.listItems, 0.7,
					{y = final},
					{delay = 0.01, ease = easing.outQuartic, onChange = function() self:updateRender() end}
				)
			end
		else
			-- nothing here
	...
	local timePassed = event.deltaTime * 250 -- 300
	local li = self.listItems
	-- Slow the list down
	velocity = velocity * self.cfg.friction
	li:setY(math.floor(li:getY() + velocity * timePassed))
	...
	if lastItem.id == #self.itemData and self.listHeight - lastItem:getY() - lastItem:getHeight() <= 0 then
		if self.listHeight > self.viewSize and li:getY() < -self.listHeight + self.viewSize - lastItem:getHeight() * 0.5 then
			...
			self.tween = GTween.new(
				li, 0.4,
				{y = math.ceil(self.viewSize - self.listHeight)},
				{delay = 0.01, ease = easing.outQuartic}
			)
...
function ListView:goToItem(xitem) -- mokalux 20191226
	local li = self.listItems
	li:setY(-math.floor(li:getChildAt(xitem):getY())) -- XXX
	self:updateRender()
